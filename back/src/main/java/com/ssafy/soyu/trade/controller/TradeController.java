...
public class TradeController {
  ...
  @GetMapping("/dp/code/{itemId}")
  @Operation(summary = "DP 코드 조회(판매자)", description = "사용자 ID와 아이템 ID를 이용해 회수 코드를 조회합니다.")
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "회수 코드 조회 성공"),
      @ApiResponse(responseCode = "400", description = "회수 코드 조회 실패")
  })
  public ResponseEntity<?> displayCode(HttpServletRequest request, @PathVariable("itemId") Long itemId) {
    Long memberId = (Long) request.getAttribute("memberId");

    String code = tradeService.getDisplayCode(memberId, itemId);
    return getResponseEntity(SuccessCode.OK, code);
  }
